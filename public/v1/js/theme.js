...
jQuery(document).ready(function($)
	{
		...
		*/
		...
		function calculateContentHeight(){
			...
			$('.vd_content-wrapper, .vd_content-wrapper > .vd_container').css('min-height', (height > height_right ) ? (height - 100) : (height_right - 100) );
		};
...
//				$('[data-action^="fullscreen"]').addClass('active');								
		...
		$('[data-action^="submenu"]').click(function() {
			...
			if($(".left-menubar").is(":visible")){
                $(".left-menubar").css("display", "none");
			}else{
                $(".left-menubar").css("display", "block");
			}
		});	
